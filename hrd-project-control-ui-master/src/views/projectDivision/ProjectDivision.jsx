...
import { getProjectDivi } from "../../redux/actions/DailyReport/dailyreportAction";
...
class ProjectDivision extends PureComponent {
  ...
  handleFilter = () => {
    ...
    this.props
      .getProjectDivi(this.state.course, this.state.generation)
      .then(() =>
        this.setState({
          isLoading: false,
          empty: false,
  ...
  render() {
    return (
      <>
        <h3>
          {" "}
          <i height={25} className="fab fa-product-hunt mb-2"></i> View Project
          Division
        </h3>
        <div>
                    ...
                    <div className="row">
                      {this.state.isLoading !== true ? (
                        this.props.dailyreport.map((datas, i) => (
                          <Link
                            to={`/project_division/view_projects_division/${datas.id}`}
                            className="col-lg-3 col-md-6 col-sm-12 mt-lg-2 mt-2 text-decoration-none"
                          >
                            <div className="bck" key={i}>
                              <div className="row">
                    ...
                    {this.state.isLoading !== false ? (
                      this.props.userProject.map((data, i) =>
                        this.props.checkProject
                          ? data.courseId === 2 && (
                              <Link
                                to={`/project_division/view_projects_division/${data.projectId}`}
                                className="col-lg-3 col-md-6 col-sm-12 mt-lg-2 mt-2 text-decoration-none"
                              >
                                <div className="bck ">
                                  <div className="row">
                                    ...
                                    <div
                                      className="col-lg-9 col-sm-9 col-9 c-2 text-white"
                                      title={data.name}
                                    >
                                      {data.name}
                                    </div>
                          ...
                          : data.courseId === 1 && (
                              <Link
                                to={`/project_division/view_projects_division/${data.projectId}`}
                                className="col-lg-3 col-md-6 col-sm-12 mt-lg-2 mt-3 text-decoration-none"
                              >
                                <div className="bck ">
                                  <div className="row">
                                    <div className="col-lg-3 col-sm-3 col-3 c-1 text-white">
...
const mapStateToProps = (state) => {
  return {
    dailyreport: state.dailyReportReducer.ProjectDivi,
